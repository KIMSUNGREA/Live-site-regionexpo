...
class Template_
{
	...
	function _define($fid, $path)
	{
		...
		default : $this->tpl_[$fid]=array('html', $path);
		}
	}
	function assign($arg)
	{
	...
	function clearCache()
	{
		...
		if (!func_num_args()) $this->_rmdir($this->cache_dir);
		else foreach (func_get_args() as $dir) $this->_rmdir($this->cache_dir.'/%clear/'.$dir);
	}
	function _rmdir($path)
	{
		...
		while ($f = @$d->read()) {
			switch ($f) {
			case '.': case '..': break;
			default : @is_dir($f=$path.'/'.$f) ? $this->_rmdir($f) : @unlink($f);
			}
		}
	}
...
// About xprint() and xfetch(), refer http://www.xtac.net/bbs/?prc=read&idx=1091
	function xprint($file, $type='html') 
	{ 
		$this->define(($type=='txt' ? '!*' : '*'), $file);
		$this->print_('*'); 
	} 
	function xfetch($file, $type='html') 
	{ 
		$this->define(($type=='txt' ? '!*' : '*'), $file);
		return $this->fetch('*'); 
	} 
