...
//파일명
$file_name = iconv("UTF-8","EUC-KR","netzeroexpo_data2_".date("Ymdhis").".xls");
...
<table>
	...
	<tbody>
	<?php
	$_list = $dbo->fetch_list("SELECT * FROM RT_APPLICATION2 ORDER BY seq DESC");
	...
	<tr>
		...
		<td><?php
			$addstr = ($_list[$m]['booth'] > 9)?"[순환경제] ":"[탄소중립] ";
			if ($_list[$m]['booth'] == 4) {
				echo "기타 : ".$_list[$m]['booth_txt'];
			} else {
				echo $addstr.$_booth[$_list[$m]['booth']];
			}
		?></td>
		<!-- <td><?php echo $_list[$m]['booth1'];?></td> -->
		<?php
		...
		if ($_list[$m]['seminar_en']=="y") {
			if ($_list[$m]['seminar'] == 1) {echo "5월 26일(목) 오전/ 탄소기술 기술이전·사업화 우수사례";}
			if ($_list[$m]['seminar'] == 2) {echo "5월 26일(목) 오후/ 탄소기술 기술이전·사업화 우수사례";}
			if ($_list[$m]['seminar'] == 3) {echo "5월 27일(금) 오전/ 新기술 및 제품소개";}
			if ($_list[$m]['seminar'] == 4) {echo "5월 27일(금) 오후/ 新기술 및 제품소개";}
		}
		?></td>
