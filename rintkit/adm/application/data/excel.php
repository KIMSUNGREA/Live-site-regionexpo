...
//파일명
$file_name = iconv("UTF-8","EUC-KR","netzeroexpo_data1_".date("Ymdhis").".xls");
...
<table>
	...
	<tbody>
	<?php
	$_list = $dbo->fetch_list("SELECT * FROM RT_APPLICATION ORDER BY seq DESC");
	...
	<tr>
		...
		<td><?php echo $ap_gubun[$_list[$m]['gubun']];?></td>
		<td><?php echo $ctr[$_list[$m]['country']];?></td>
		...
		<td>
			<?php
				if ($_list[$m]['if_3_1']=="y") {$if_3[] = "5월 25일(수)";}
				if ($_list[$m]['if_3_2']=="y") {$if_3[] = "5월 26일(목)";}
				if ($_list[$m]['if_3_3']=="y") {$if_3[] = "5월 27일(금)";}
		...
		<td>
			<?php
				if ($_list[$m]['sv_3_1']=="y") {$sv_3[] = "산업정책 및 기업지원";}
				if ($_list[$m]['sv_3_2']=="y") {$sv_3[] = "기술이전/사업화";}
				if ($_list[$m]['sv_3_3']=="y") {$sv_3[] = "금융,세제";}
				if ($_list[$m]['sv_3_4']=="y") {$sv_3[] = "탄소중립 선도기업";}
				if ($_list[$m]['sv_3_5']=="y") {$sv_3[] = "신사업";}
				if ($_list[$m]['sv_3_6']=="y") {$sv_3[] = "미래 모빌리티";}
				if ($_list[$m]['sv_3_7']=="y") {$sv_3[] = "에너지효율";}
				if ($_list[$m]['sv_3_8']=="y") {$sv_3[] = "글로벌";}
				if ($_list[$m]['sv_3_9']=="y") {$sv_3[] = "자원순환";}
