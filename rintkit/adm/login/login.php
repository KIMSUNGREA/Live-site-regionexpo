		...
		<div class="rt_login_in">
			<div class="rt_login_con">
				<form name="login_form" action="<?php echo RTW_ADM;?>/login/update.php" method="post" target="rt_ifrm">
					...
					<h1 class="rt_tit">관리자 로그인</h1>
					<p class="rt_txt">Sign in using your registered account</p>
					<div class="rt_input_box">
						<input type="text" name="adm_id" value="<?php echo (empty($env->_cookie['save_adm_id']))?"":$env->_cookie['save_adm_id']?>" id="adm_id" class="rt_input la required" placeholder="ID" msg="아이디를 정확히 입력해 주세요">
					</div>
					<div class="rt_input_box">
						<input type="password" name="adm_pw" id="adm_pw" class="rt_input la required" value="" placeholder="PW" msg="비밀번호를 정확히 입력해 주세요">
					</div>
					<div class="rt_check_box">
						<div class="chk_list">
											<input type="checkbox" name="is_memory" id="is_memory" value="y" class="chk_li" <?php echo $is_save_id;?>>
											<label for="is_memory" class="chk_la"> ID 기억하기<span class="icon chk_icon"></span></label>
										</div>
					...
					<div class="bt-wrap">
						<button type="button" class="rt_btn dark icon block s-r" id="btnSubmit" style="cursor: pointer;">관리자 접속하기 &nbsp;&nbsp;<img src="/rintkit/adm/layout/images/common/login_arr.png" alt=""></button>
					</div>
				</form>
			</div>
			<p>Copyright &copy;ilclinic.co.kr All Rights reserved.</p>
		</div>
	</div>
</div>
