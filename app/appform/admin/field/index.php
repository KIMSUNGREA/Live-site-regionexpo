...
switch ($_def['func'])
{
	case "field"	:
		...
		//전체 필드 정보
		$_fl = $dbo->fetch_list("SELECT * FROM RT_APPFORM_ADD_FIELD WHERE bcode='".$env->_get['bcode']."' ORDER BY formnum ASC");

		//필드 정보
		$_f = $dbo->fetch("SELECT * FROM RT_APPFORM_ADD_FIELD WHERE seq='".$env->_get['seq']."'");
