...
<div class="rt_search_wrap rt_tar">
	<div class="rt_button_wrap rt_fll">
		<select name="" id="bcode" onchange="document.move_form.bcode.value=$(this).val();">
			...
			<?php 
			for ($m = 0 ; $m <count($_board_qna) ; $m++) { 
				if ($_board_qna[$m]['bcode'] != $env->_get['bcode']) { ?>
					<option value="<?php echo $_board_qna[$m]['bcode'];?>"><? echo $_board_qna[$m]['name'];?></option>
				<?php 
				}
			} ?>
		</select>
...
<table class="rt_list_table">
	...
	<tbody>
		...
		<tr>
			...
			<?if ($_conf['category_is'] == "y") {?>
			<td><p><?php echo $_l[$i]['category'];?></td>
			<?}?>
			<td class="rt_tal">
				<p>
					<a href="<?php echo $_l[$i]['viewpage'];?>"><?php echo ($_l[$i]['use_is'] == "n") ? "<span style='color:red'>[노출제한] </span>" : "";?><?php echo $_l[$i]['subject'];?></a>
			...
			<td><?php echo $_l[$i]['name'];?></td>
			<td><?php echo $_l[$i]['reg_date'];?></td>
			<td><?php echo ($_l[$i]['reply_is']=="y")?"<span style='color:blue;'>답변완료</span>":"<span style='color:red'>답변대기</span>";?></td>
		</tr>
		<?php
			}
		} else {
...
<div class="rt_button_wrap clearfix">
	...
	<div class="rt_flr">
		<a href="<?php echo RTW_ADM?>/app/?appcode=rtboard&sd=theme-qna-data&cf=form&bcode=<?php echo $env->_get['bcode'];?>" class="rt_btn_blue">글 쓰기</a>
	</div>
</div>
