...
if ($env->_post['mode'] == "insert")
{
	...
	if($cls_board->insert($udata)) {
		...
		rt_js_move("", "parent", "href", RTW_ADM."/app/?appcode=rtboard&sd=theme-qna-data&bcode=".$env->_post['bcode']);
	} else {
...
else if($env->_post['mode'] == "modify" && is_numeric($env->_post['seq']))
{
	...
	if ($cls_board->update($udata, "seq='".$env->_post['seq']."'")) {
		...
		rt_js_move("", "parent", "href", RTW_ADM."/app/?appcode=rtboard&sd=theme-qna-data&bcode=".$env->_post['bcode']."&cf=view&seq=".$env->_post['seq'].$add_url);
	} else {
		rt_js_msg("시스템문제로 등록되지 않았습니다.");
	}
}
else if($env->_post['mode'] == "reply" && is_numeric($env->_post['seq']))
{
	//게시판 환경 정보
	$_conf = $dbo->fetch("SELECT * FROM ".$cls_board->db_tbl."_CONF WHERE bcode='".$env->_post['bcode']."'");
	...
	//DB 입력 데이터 설정
	$reply_is = ($env->_post['reply_is']=="y") ? "y":"n";
	$reply_date = ($env->_post['reply_date']) ? $env->_post['reply_date'] : date("Y-m-d H:i:s");

	$udata['reply_is'		] = $reply_is;
	$udata['content_reply'	] = $env->_post['content_reply'];
	$udata['reply_date'		] = $reply_date;
...
else if($env->_get['mode'] == "delete" && is_numeric($env->_get['seq']))
{
	...
	if ($dbo->query("DELETE FROM ".$cls_board->db_tbl." WHERE seq='".$r['seq']."'")) {
		...
		rt_js_move("", "parent", "href", RTW_ADM."/app/?appcode=rtboard&sd=theme-qna-data&bcode=".$env->_get['bcode'].$add_url);
	} else {
		rt_js_msg("시스템문제로 등록되지 않았습니다.");
	}
}
