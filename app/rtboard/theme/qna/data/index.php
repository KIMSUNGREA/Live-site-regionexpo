...
$cls_board = new rtboard_qna($_conf['page_cnt_admin'],$_conf['block_cnt_admin']);
...
switch ($_def['func'])
{
	case "list"	:
		...
		//공지사항 데이터 설정
		$_ntc = $dbo->fetch_list("SELECT * FROM RT_RTBOARD_QNA WHERE bcode='".$env->_get['bcode']."' AND notice_is='y' ORDER BY seq DESC");
		if (count($_ntc) > 0) {
		...
		$_board_qna = $dbo->fetch_list("SELECT * FROM RT_RTBOARD WHERE theme='qna' ORDER BY bseq ASC");
	break;
	...
	case "reply"	:
		...
		if($env->_get['seq']){
			...
			${"reply_is_".$r['reply_is']} = "checked='checked'";
			${"reply_is_class_".$r['reply_is']} = "class='check'";
			$r['content'] = ($_conf['webeditor_is'] == "y")?rt_dbstr_decode($r['content']):nl2br($r['content']);
			$r['reply_date'] = substr($r['reply_date'],0,10);
	...
	case "view"	:
		...
		if (is_numeric($env->_get['seq'])) {
			...
			$r['content'] = ($_conf['webeditor_is'] == "y")?rt_dbstr_decode($r['content']):nl2br($r['content']);
			$r['content_reply'] = ($_conf['webeditor_is'] == "y")?rt_dbstr_decode($r['content_reply']):nl2br($r['content_reply']);
