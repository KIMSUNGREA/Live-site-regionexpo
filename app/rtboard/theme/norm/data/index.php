...
$cls_board = new rtboard_norm($_conf['page_cnt_admin'],$_conf['block_cnt_admin']);
...
switch ($_def['func'])
{
	case "list"	:
		...
		//공지사항 데이터 설정
		$_ntc = $dbo->fetch_list("SELECT * FROM RT_RTBOARD_NORM WHERE bcode='".$env->_get['bcode']."' AND notice_is='y' ORDER BY seq DESC");
		if (count($_ntc) > 0) {
		...
		if ($cls_board->list_rec_cnt > 0)
		{
			...
			for ($i = 0; $i < count($_l); $i++)
			{
				...
				if ($_l[$i]['re_level'] > 0) {
					...
					$_l[$i]['reply_width'] = "<img src='".$_def['path_theme']."/user/files/ico_level.gif' width=".$wid." height='8'>";
				} else {
					$_l[$i]['reply_width'] = "";
				}
		...
		$_board_norm = $dbo->fetch_list("SELECT * FROM RT_RTBOARD WHERE theme='norm' ORDER BY bseq ASC");
	break;
	...
	case "view"	:
		...
		if (is_numeric($env->_get['seq'])) {
			...
			$r['content'] = ($r['webeditor_is'] == "y")?rt_dbstr_decode($r['content']):nl2br($r['content']);
