...
switch ($_def['func'])
{
	case "auth"	:
		...
		//게시판 환경 정보
		$_r = $dbo->fetch("SELECT * FROM RT_RTBOARD_NORM_CONF WHERE bcode='".$env->_get['bcode']."'");
		...
		$auth_reply_arr = unserialize(html_entity_decode($_r['auth_reply']));
