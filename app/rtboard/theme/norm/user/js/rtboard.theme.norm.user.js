...
function post_my_modify() {
	document.my_modify_form.submit();
}
...
function rt_get_capcha()
{
	...
	if (form.mode.value == "insert" || form.mode.value == "reply") {
...
;(function($) {
	$(function() {
		...
		$("#btn-reply").click(function () {
			...
			var search = form.search.value;
			var searchstring = form.searchstring.value;
			location.href = "?pg="+pg+"&search="+search+"&searchstring="+searchstring;
		}).css("cursor","pointer");
		...
		$('.rt-rwd-reply-comment,.rt-reply-comment').click(function(){
			$(this).parent().siblings().children('.rt-rwd-reply-toggle').slideUp();
			$(this).next().slideToggle();
		});
		...
		$('.rt-img-attach-btn a').on('click', function(){
			var tmp = $(this).index();
			$('.rt-img-attach-list-wrap').each(function(){
				$(this).children().eq(tmp).show().siblings('.rt-img-attach-list-con').hide();
			});
